...
fun ManageServiceScreen(viewModel: ShopOwnerViewModel) {
    ...
    var servicePrice by remember { mutableIntStateOf(500) }
    val shopId = 1
    ...
    LazyColumn(
        ...
        item {
            Text("Thêm Dịch Vụ", fontSize = 20.sp, fontWeight = FontWeight.Bold)
            OutlinedTextField(
                value = serviceName,
            ...
            OutlinedTextField(
                ...
                onValueChange = { servicePrice = it.toIntOrNull() ?: 0 },
                label = { Text("Giá (VND)") },
        ...
        items(viewModel.uiState.services) { service ->
            Card(
                modifier = Modifier
                    .fillMaxWidth()
